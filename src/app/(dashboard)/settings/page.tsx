...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (profile) {
      setFormData(prev => ({
        ...
        phone: profile.phone || "",
        // @ts-ignore
        document: profile.document || "", // 👈 PUXA O CPF DO BANCO
        // @ts-ignore
        bio: profile.bio || "",
        // @ts-ignore
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const settingsPayload = {
      ...
      document: formData.document, // 👈 ENVIA O CPF PARA A API
      bio: formData.bio,            
              ...
              <motion.div variants={tabContentVariants} initial="hidden" animate="visible" className="grid gap-6 lg:grid-cols-[300px_1fr]">
                ...
                <Card className="rounded-3xl border border-border/50 bg-card shadow-sm p-6 sm:p-8 transition-all hover:shadow-md">
                  <div className="space-y-6 sm:space-y-8">
                    ...
                    {/* 👇 O NOME DA EMPRESA OCUPA A LARGURA TOTAL AQUI 👇 */}
                    ...
                    {/* 👇 WHATSAPP E CPF/CNPJ DIVIDEM O ESPAÇO NA MESMA LINHA 👇 */}
