...
export default function SettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const settingsPayload = {
      ...
      document: formData.document, bio: formData.bio, requirePixDeposit: formData.requirePixDeposit,
  ...
  // 👇 NOVA FUNÇÃO: Troca de Planos
  const handleChangePlan = async (newPlan: string) => {
    if (!confirm(`Tem a certeza que deseja mudar para o plano ${newPlan}? O novo valor será aplicado na sua próxima fatura.`)) return;
    
    try {
      setIsChangingPlan(true);
      await api.post('/billing/plan', { plan: newPlan });
      toast.success(`Plano atualizado para ${newPlan} com sucesso!`);
      // Recarrega a página para atualizar as permissões da interface
      ...
      <form onSubmit={handleSubmit}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Tabs defaultValue="perfil" className="space-y-8 w-full">
            ...
            <div className="w-full overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:pb-0 [&::-webkit-scrollbar]:hidden">
              <div className="w-max sm:w-fit rounded-2xl bg-muted/40 p-1.5 border border-border/50 shadow-sm">
                <TabsList className="bg-transparent p-0 h-auto gap-1.5 w-full flex">
                  <TabsTrigger value="perfil" onClick={handleTabClick} className="flex-1 sm:flex-none sm:min-w-30 rounded-xl py-2.5 px-5 text-sm font-bold data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all data-[state=active]:text-primary flex items-center gap-2"><User className="h-4 w-4" /> Perfil</TabsTrigger>
                  <TabsTrigger value="vitrine" onClick={handleTabClick} className="flex-1 sm:flex-none sm:min-w-30 rounded-xl py-2.5 px-5 text-sm font-bold data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all data-[state=active]:text-primary flex items-center gap-2"><Store className="h-4 w-4" /> Vitrine</TabsTrigger>
                  <TabsTrigger value="whatsapp" onClick={handleTabClick} className="flex-1 sm:flex-none sm:min-w-30 rounded-xl py-2.5 px-5 text-sm font-bold data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all data-[state=active]:text-green-600 text-muted-foreground flex items-center gap-2"><MessageCircle className="h-4 w-4" /> WhatsApp</TabsTrigger>
                  <TabsTrigger value="pagamentos" onClick={handleTabClick} className="flex-1 sm:flex-none sm:min-w-30 rounded-xl py-2.5 px-5 text-sm font-bold data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all text-amber-600/70 data-[state=active]:text-amber-600 flex items-center gap-2"><DollarSign className="h-4 w-4" /> Financeiro</TabsTrigger>
                  <TabsTrigger value="seguranca" onClick={handleTabClick} className="flex-1 sm:flex-none sm:min-w-30 rounded-xl py-2.5 px-5 text-sm font-bold data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all data-[state=active]:text-primary flex items-center gap-2"><Lock className="h-4 w-4" /> Segurança</TabsTrigger>
                  {isAdmin && (
            ...
            <TabsContent value="perfil" className="outline-none">
              <motion.div variants={tabContentVariants} initial="hidden" animate="visible" className="grid gap-6 lg:grid-cols-[300px_1fr]">
                <Card className="rounded-3xl border border-border/50 bg-card shadow-sm p-6 flex flex-col items-center justify-center text-center gap-4 transition-all hover:shadow-md hover:border-primary/20">
                  ...
                  {formData.avatarUrl && <Button type="button" variant="ghost" size="sm" className="text-destructive text-xs hover:bg-destructive/10 font-bold mt-1" onClick={() => setFormData(prev => ({ ...prev, avatarUrl: "" }))}><Trash2 className="h-3 w-3 mr-2" /> Remover Foto</Button>}
                </Card>
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2.5">
                        ...
                        <input value={formData.document} onChange={(e) => setFormData({...formData, document: e.target.value.replace(/\D/g, '')})} className={`w-full ${inputStyle}`} placeholder="Apenas números" maxLength={14} />
                      </div>
                    </div>
                    <div className="space-y-2.5">
                          ...
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                            <div className="space-y-8 p-6 mt-2 rounded-2xl bg-amber-500/5 border border-amber-500/20 shadow-inner">
                              ...
                              <div className="space-y-4">
                                ...
                                <input type="password" placeholder="APP_USR-..." value={formData.mercadoPagoAccessToken} onChange={(e) => setFormData({...formData, mercadoPagoAccessToken: e.target.value})} className={`w-full ${inputStyle} font-mono text-sm h-12 border-amber-500/30 focus:border-amber-500 focus:ring-amber-500/20`} />
                              </div>
                            </div>
                          </motion.div>
                        )}
            ...
            {/* 👇 CONTEÚDO: ASSINATURA (COM A ESCOLHA DE PLANOS) 👇 */}
            {isAdmin && (
                ...
                <motion.div variants={tabContentVariants} initial="hidden" animate="visible" className="max-w-3xl space-y-6">
                  ...
                  <Card className="rounded-3xl border border-purple-500/20 shadow-lg overflow-hidden ring-1 ring-purple-500/10">
                    ...
                    <div className="p-6 sm:p-8 space-y-8 bg-card">
                      ...
                      {/* 👇 A NOVA SEÇÃO DE ESCOLHA DE PLANOS 👇 */}
